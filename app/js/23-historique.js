...
function ouvrirHistoriquePersonnel(roleDemande) {
  ...
  if (!ov) {
    ...
    ov.className = 'hidden fixed inset-0 modal-overlay z-50 flex items-end';
    ov.style.zIndex = '400';
    ov.innerHTML =
      '<div class="modal-content w-full bg-white p-6 rounded-t-3xl" style="max-height:92vh;overflow-y:auto;">' +
        '<div class="flex items-center justify-between mb-3">' +
          '<h2 class="text-2xl font-bold text-gray-800" id="histo-titre">Historique des absences</h2>' +
          '<button onclick="fermerHistoriquePersonnel()" class="text-gray-400 hover:text-gray-600 text-2xl leading-none">&times;</button>' +
        '</div>' +
        '<div id="histo-filtres" class="flex flex-wrap gap-2 mb-3"></div>' +
        '<div id="histo-totaux" class="mb-3"></div>' +
        '<div id="histo-liste"></div>' +
        '<div class="flex gap-3 mt-4">' +
          '<button onclick="fermerHistoriquePersonnel()" class="btn-fermer flex-1">Fermer</button>' +
        '</div>' +
      '</div>';
    document.body.appendChild(ov);
  }
  // les listes deroulantes : memes classes que partout dans l'application
