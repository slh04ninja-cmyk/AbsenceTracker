...
async function serveurEnvoyerFiches() {
    ...
    try {
      if (!existante) {
        ...
        if (cree && cree[0]) {
          ...
          memoire[cree[0].id] = 1;              // c'est NOUS qui l'avons creee
          Depot.ecrireJSON('fichesCreeesParApp', memoire);
          rapport.fiches++;
        }
  ...
  const memoire = Depot.lireJSON('fichesCreeesParApp', {}) || {};
