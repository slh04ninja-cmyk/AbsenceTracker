...
function dump(t){
  conteneur.enfants = [];
  afficherListeEleves();
  const r = conteneur.enfants;
  const html1 = r[0].innerHTML.replace(/\s+/g,' ');
  const html2 = r[1].innerHTML.replace(/\s+/g,' ');
  console.log(t,
    '| bordure cochee:', (r[0].style.match(/border-left: 5px solid (#[0-9a-fA-F]{6})/)||[])[1],
    '| nom coche:', (html1.match(/nowrap; (color: #[0-9a-fA-F]{6}; font-weight: 600)/)||[])[1] || (html1.match(/color: (#[0-9a-fA-F]{6}); font-weight: 600/)||[])[1],
    '| nom normal:', (html2.match(/nowrap; (color: #[0-9a-fA-F]{6};)/)||[])[1] || (html2.match(/nowrap; (color: #[0-9a-fA-F]{6})/)||[])[1],
    '| pastille:', (html2.match(/yes; (background: [^;]+; color: [^;]+;)/)||[])[1]);
}
dump('CLAIR ');
