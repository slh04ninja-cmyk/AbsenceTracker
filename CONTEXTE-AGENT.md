# CONTEXTE-AGENT.md — AbsenceTrack
...
*(Nom standard attendu par certains outils : `AGENTS.md` — ce fichier en tient lieu ; voir la note en fin de document.)*
...
longue chaîne `&&` : écrire `> /dev/null` ou enchaîner avec `|`).
...
- Le dépôt contient deux lignées : **`android-v4`** (l'application — **branche par défaut**) et
  `main` (les premières migrations serveur). Elles ont divergé ; ne pas les fusionner à la légère.
...
Le nom **`AGENTS.md`** (lu automatiquement par plusieurs agents) n'a pas pu être créé depuis cette
machine : l'écriture de ce nom de fichier y est protégée et demande une validation explicite du
propriétaire. Si tu lis ce document sous le nom `CONTEXTE-AGENT.md`, sache que son contenu est
prévu pour être renommé/copié en `AGENTS.md` à la racine — c'est la seule différence.
