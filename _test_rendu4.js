...
function dump(t){
  ...
  const get = (row) => (row.innerHTML.replace(/\s+/g,' ').match(/white-space: nowrap; ([^"]*)"/)||[])[1] || (row.innerHTML.replace(/\s+/g,' ').match(/nowrap; (color: #[0-9a-fA-F]{6})/)||[])[1];
  console.log(t, '| ligne cochee:', r[0].style.match(/border-left: 5px solid (#[0-9a-fA-F]{6})/)[1], '| nom coche:', (r[0].innerHTML.match(/color: (#[0-9a-fA-F]{6}); font-weight: 600/)||[])[1]);
  console.log('   ligne normale:', get(r[1]));
}
dump('CLAIR ');
