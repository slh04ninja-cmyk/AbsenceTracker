...
function fermerHistoriqueEleve() {
  document.getElementById('modal-historique-eleve').classList.add('hidden');
}
...
function afficherClassesSurv() {
  ...
  div.innerHTML = '';
  // Trouver les élèves avec des absences justifiées
  ...
  liste.forEach(x => {
    ...
    item.onclick = () => ouvrirHistoriqueEleve(x.nom, x.classe);
    item.innerHTML = `
      <div>
...
function ouvrirHistoriqueEleve(nom, classe) {
  const lignes = absences.filter(a => a.nom === nom && a.classe === classe && (a.statut === 'justifie_s' || a.statut === 'justifie_d'));
  document.getElementById('historique-eleve-titre').textContent = nom + ' — ' + classe;
  const tbody = document.getElementById('historique-eleve-body');
  tbody.innerHTML = '';
  lignes.sort((a, b) => String(b.dateISO || '').localeCompare(String(a.dateISO || '')));
  lignes.forEach(a => {
    const tr = document.createElement('tr');
    tr.innerHTML = '<td class="px-3 py-2 text-sm">' + (a.date || '') + '</td><td class="px-3 py-2 text-sm">' + (a.heure || '') + '</td><td class="px-3 py-2 text-sm">' + (a.matiere || '') + '</td><td class="px-3 py-2 text-sm">' + (a.enseignant || '') + '</td>';
    tbody.appendChild(tr);
  });
  document.getElementById('modal-historique-eleve').classList.remove('hidden');
}
