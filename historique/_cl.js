...
function afficherListeEleves() {
  ...
  tousLesEleves.forEach(eleve => {
    ...
    const couleur = cocheRetard ? '#74759C' : '#D93C78';
    const fond = cocheRetard ? 'rgba(116,117,156,0.16)' : 'rgba(217,60,120,0.09)';
    ...
    const styleChip = cocheRetard ? 'background: #74759C; color: #fff;' : (estCoche ? 'background: #D93C78; color: #fff;' : 'background: #AAAAD0; color: #6994CC;');
    const styleNom = cocheRetard ? 'color: #74759C; font-weight: 600;' : (estCoche ? 'color: #363759; font-weight: 600;' : 'color: #363759;');
...
function mettreAJourDashboardSurv() {
  ...
  if (absentsListe.length === 0) {
    listContainer.innerHTML = '<div style="text-align: center; padding: 32px 16px; color: #74759C;"><i class="fas fa-user-check" style="font-size: 32px; margin-bottom: 12px; display: block; opacity: 0.4;"></i><p>Aucun élève signalé aujourd\'hui</p></div>';
  } else {
...
function afficherDetailAbsence(abs) {
  ...
  const couleurType = estRetard ? '#74759C' : (estExclusion ? '#74759C' : '#D93C78');
...
function ouvrirFicheEleve(eleveId, classeId) {
  ...
  if (lignes.length === 0) {
    ...
    cont.innerHTML = tri.map(a => {
      ...
      const styleMarque = retard ? 'color: #74759C; font-weight: 800;' : 'color: #D93C78; font-weight: 800;';
